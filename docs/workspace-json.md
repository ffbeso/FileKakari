...
Workspaceは、任意の名前の`*.workspace.json`ファイルとして保存できます。

保存したWorkspace JSONは、FileKakariから明示的に開いて使用します。特定のファイル名によるフォルダ単位の自動検出は行いません。

## パス

Workspace内のフォルダパスは、`rootPath`を基準とした相対パス、または絶対パスとして保存できます。

`rootPath`を設定すると、Workspaceを別の環境へ移動した場合でも、同じ相対構成を維持しやすくなります。

## セッション状態

選択中のファイル、スクロール位置、フィルター、個人ごとの列幅などの実行時状態は、Workspace JSONには保存しません。

これらはアプリのセッション状態として保持されます。
